window.albums = [
  {
    name: 'Eitan Reiter',
    artist: 'Remix',
    image: 'https://anapy34.github.io/music-player/Eitan_Reiter/Eitan_Reiter_remix.jpg',
    songs: [
      {
        name: 'Ups And Downs',
        url: 'https://anapy34.github.io/music-player/Eitan_Reiter/Ups_%26_Downs_%20Eitan_Reiter.mp3'
      },
    
    ],
  },

  {
    name: 'Engenheiro do Hawaii',
    artist: 'Acustico',
    image: 'https://i.scdn.co/image/ab67616d0000b273f98de08cd017b515c9e61344',
    songs: [
      {
        name: 'Armas Quimicas e Poemas',
        url: './Engenheiros_do_Hawaii/05-ArmasQuimicasePoemas.mp3'
      },
      {
        name: 'Vida Real',
        url: './Engenheiros_do_Hawaii/03-VidaReal.mp3'
      },
      {
        name: 'Ate o Fim',
        url: './Engenheiros_do_Hawaii/02-AteoFim.mp3'
      },
      {
        name: 'O Preco',
        url: './Engenheiros_do_Hawaii/06 - O Preço.mp3'
      },
    ],
  },

  {
    name: 'cicero',
    artist: 'canções de Apartamento',
    image: 'https://i.scdn.co/image/ab67616d0000b273bad7f5839b4b166fd8b323e5',
    songs: [
      {
        name: 'tempo de pipa',
        url: 'https://anapy34.github.io/video-player/cicero/Can%C3%A7%C3%B5es%20De%20Apartamento%20MP3/01%20Tempo%20De%20Pipa.mp3'
      },
      {
        name: 'Vagalumes Cegos',
        url: 'https://anapy34.github.io/video-player/cicero/Canções De Apartamento MP3/02 Vagalumes Cegos.mp3'
      },
      {
        name: 'make-me-move',
        url: 'https://ncsmusic.s3.eu-west-1.amazonaws.com/tracks/000/000/286/make-me-move-feat-karra-1586948725-C9SPOA3UZn.mp3'
      },
      {
        name: 'Christmas Rap',
        url: 'http://incompetech.com/music/royalty-free/mp3-royaltyfree/Christmas%20Rap.mp3'
      },
      {
        name: 'Dont Strat Now',
        url: 'https://rolandhu.github.io/lyrics-synchronization/dont-start-now.mp3'
      },
    ],
  },

  {
    name: 'akd',
    artist: 'Progressing',
    image: 'https://f4.bcbits.com/img/a2095838675_10.jpg',
    songs: [
      {
        name: 'Archaic Myth',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/01%20-%20Akd%20-%20Archaic%20Myth.mp3'
      },
      {
        name: 'Evolving Spirits',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/03%20-%20Akd%20-%20Evolving%20Spirits.mp3'
      },
      {
        name: 'Crystal Storm Breath',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/04%20-%20Akd%20-%20Crystal%20Storm%20Breath.mp3'
      },
      {
        name: 'Tender Elevation',
        url:' https://anapy34.github.io/video-player/akd%20progressing/07%20-%20Akd%20-%20Tender%20Elevation.mp3'
      },
      {
        name: 'New Style Trance',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/08%20-%20Akd%20-%20New%20Style%20Trance.mp3'
      },
    ],
  },
  
  {
    name: 'Radio do Senado Brasileiro',
    artist: 'Senado Federal',
    image: 'https://www.cxradio.com.br/img/Radio/Logo/338f189670306576ab3d1e4513479ce7.jpg',
    songs: [
      {
        name: 'Radio Senado',
        url: 'https://radioaovivo.senado.leg.br/fm.mp3'
      },
    ],
  },

  {
    name: 'Engenheiro do Hawaii',
    artist: 'Acustico',
    image: 'https://i.scdn.co/image/ab67616d0000b273f98de08cd017b515c9e61344',
    songs: [
      {
        name: 'Armas Quimicas e Poemas',
        url: './Engenheiros_do_Hawaii/05-ArmasQuimicasePoemas.mp3'
      },
      {
        name: 'Vida Real',
        url: './Engenheiros_do_Hawaii/03-VidaReal.mp3'
      },
      {
        name: 'Ate o Fim',
        url: './Engenheiros_do_Hawaii/02-AteoFim.mp3'
      },
      {
        name: 'O Preco',
        url: './Engenheiros_do_Hawaii/06 - O Preço.mp3'
      },
    ],
  },
  {
    name: 'akd',
    artist: 'Progressing',
    image: 'https://f4.bcbits.com/img/a2095838675_10.jpg',
    songs: [
      {
        name: 'Archaic Myth',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/01%20-%20Akd%20-%20Archaic%20Myth.mp3'
      },
      {
        name: 'Evolving Spirits',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/03%20-%20Akd%20-%20Evolving%20Spirits.mp3'
      },
      {
        name: 'Crystal Storm Breath',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/04%20-%20Akd%20-%20Crystal%20Storm%20Breath.mp3'
      },
      {
        name: 'Tender Elevation',
        url:' https://anapy34.github.io/video-player/akd%20progressing/07%20-%20Akd%20-%20Tender%20Elevation.mp3'
      },
      {
        name: 'New Style Trance',
        url: 'https://anapy34.github.io/video-player/akd%20progressing/08%20-%20Akd%20-%20New%20Style%20Trance.mp3'
      },
    ],
  },  
  
];
